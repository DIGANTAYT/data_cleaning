...
export default function LoginPage() {
  ...
  const saveStateToHistory = (currEmail: string, currPassword: string) => {
    if (isApplyingHistoryRef.current) return;
    setHistory(prev => {
      const updated = prev.slice(0, historyIndex + 1);
      return [...updated, { email: currEmail, password: currPassword }];
    });
  ...
  React.useEffect(() => {
    const handler = setTimeout(() => {
      if (email || password) {
        ...
        if (!lastSnap || lastSnap.email !== email || lastSnap.password !== password) {
          saveStateToHistory(email, password);
        }
      }
    }, 400);
